...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio = 0.5):
        ...
        outputs = torch.zeros(trg_len, batch_size, trg_vocab_size).to(self.device)# tenseur pour stocker les sorties du décodeur
        
        hidden, cell = self.encoder(src) # le dernier état caché de l'encodeur est utilisé comme état caché initial du décodeur
